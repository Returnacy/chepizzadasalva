...
let cachedPrivateKey: unknown = null;
...
function getWalletConfig(): WalletConfig {
  ...
  ? allowedOriginsEnv.split(',').map((origin: string) => origin.trim()).filter((value: string) => value.length > 0)
    : [];
...
async function loadPrivateKey(privateKeyRaw: string): Promise<unknown> {
...
function buildLoyaltyObject(config: WalletConfig, params: LoyaltyPassParams) {
  ...
  const textModules: Array<{ header: string; body: string }> = [
    { header: 'Business ID', body: params.businessId },
  ];
